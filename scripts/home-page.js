...
async function checkCart() {
  ...
  for (element in productNames) {
    let category;
    if (groceryArray.includes(productNames[element].toLowerCase())) {
      category = "grocery";
    } else if (fruitsArray.includes(productNames[element].toLowerCase())) {
      category = "fruits";
    } else if (vegetablesArray.includes(productNames[element].toLowerCase())) {
      category = "vegetables";
    } else {
      category = "houseHold";
    }
    if (
      uniqueData[category][productNames[element].toLowerCase()].cart === true
    ) {
      flag = 1;
      break;
    }
...
async function directToHomePage(element) {
  removeBg();
  searchBar.value = "";
  if (element === homePage) {
  } else {
...
async function directToCart(element) {
  removeBg();
  searchBar4.value = "";
  if (element === cartPage) {
  } else {
...
async function directToProfilePage() {
  removeBg();
  searchBar7.value = "";
...
async function searchProductOtherPage(element, searchBarId, snackBarId) {
...
async function directToShoppingPage(category, element) {
  searchBar2.value = "";
  displayNone(element);
